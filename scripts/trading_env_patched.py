...
class CryptoFuturesEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.cooldown = 0
        # tracking
        self.position_step_counts = {'long': 0, 'short': 0, 'flat': 0}
        self.hold_durations = []  # hold duration in steps for closed positions
        return self._obs(), {}
    ...
    def _close(self):
        ...
        hold_steps = int(self.step_idx - self.entry_step)
        self.hold_durations.append(hold_steps)
        self.trades.append(dict(
            step=self.step_idx,
            timestamp=str(self.df.iloc[self.step_idx].get('timestamp', '')),
            action='CLOSE', price=price, size=self.position_size,
            pnl=round(pnl, 4), fee=round(fee, 4), hold_steps=hold_steps,
        ))
        self.position = 0
    ...
    def step(self, action):
        ...
        # exposure tracking (based on position held after action decision for this bar)
        if self.position == 1:
            self.position_step_counts['long'] += 1
        elif self.position == -1:
            self.position_step_counts['short'] += 1
        else:
            self.position_step_counts['flat'] += 1
    ...
    def get_metrics(self) -> dict:
        ...
        # Exposure
        total_steps = sum(self.position_step_counts.values()) or 1
        pct_long = self.position_step_counts['long'] / total_steps * 100
        pct_short = self.position_step_counts['short'] / total_steps * 100
        pct_flat = self.position_step_counts['flat'] / total_steps * 100

        # Holding
        avg_hold_steps = float(np.mean(self.hold_durations)) if self.hold_durations else 0.0

        # Fees
        total_fees = float(sum(float(t.get('fee', 0.0) or 0.0) for t in self.trades))
        avg_fee_per_closed_trade = (total_fees / len(closed)) if closed else 0.0
        ...
        return dict(
            ...
            sl_hits=sl_count,
            tp_hits=tp_count,
            pct_time_long=round(pct_long, 2),
            pct_time_short=round(pct_short, 2),
            pct_time_flat=round(pct_flat, 2),
            avg_hold_steps=round(avg_hold_steps, 2),
            total_fees=round(total_fees, 4),
            avg_fee_per_closed_trade=round(avg_fee_per_closed_trade, 6),
        )
